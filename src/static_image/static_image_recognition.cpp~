...
const int tolerance = 10; // how far away from the colour can be accepted the same colour?
...
int main(int argc, char *argv[]) {
	...
	CvScalar targetColour = cvScalar((uchar)atoi(argv[2]), (uchar)atoi(argv[3]), (uchar)atoi(argv[4]), 0);
	...
	for (int h = 0; h < clone->height; h++) {
		for (int w = 0; w < clone->width; w++) {
			CvScalar currentColour = cvGet2D(clone,h,w);
			if (currentColour.val[0] >= targetColour.val[0]-tolerance && 
